...
function draw() {
    ...
    for (var i = 0; i < pipe.length; i++) {
        ...
        gameContext.drawImage(pipeNorth, pipe[i].x, pipe[i].y);
        gameContext.drawImage(pipeSouth, pipe[i].x, pipe[i].y + constant);

        // pipe moves from right to left
        pipe[i].x--;
        ...
        if (birdX + bird.width >= pipe[i].x && birdX <= pipe[i].x + pipeNorth.width && (birdY <= pipe[i].y + pipeNorth.height || birdY + bird.height >= pipe[i].y + constant) || birdY + bird.height >= gameCanvas.height - platformImage.height) {
